...
class KNN(object):
    def __init__(self):
        self._unknown = 0 # 未知数据的向量 (1xN)
        self._train_data = 0 # M个已知数据的向量 (NxM)
        self._labels = 0 # M个已知数据的标签的向量 (1xM)
        self._k = 5 # 选择前k个最相似的数据 (k一般是不大于20的整数)
    ...
    def predict(self, unknown:list, existingdata:list, labels:list, k = 5):
        ...
        """
        self._unknown = np.array(unknown)
        self._train_data = np.array(existingdata)
        self._labels = np.array(labels)
        self._k = k

        data_set_size = self._train_data.shape[0] # 获得已知数据M的大小

        # 用tile函数使unknown扩展为NxM的矩阵，并与已知数据做减法
        diff_mat = np.tile(self._unknown, (data_set_size,1)) - self._train_data
        ...
        label_count = {} # 创建计数字典
        for i in range(self._k): # 统计距离最小的前k个标签分别的个数
            label = self._labels[indices[i]]
