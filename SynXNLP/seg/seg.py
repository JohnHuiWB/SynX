...
class Seg(object):
    ...
    def __init__(self, file_path_of_stopWords = 'all_stopword.txt'):
        ...
        self._file_path_of_stopWords = os.path.join(os.path.dirname(os.path.abspath(__file__)), file_path_of_stopWords)
    ...
    def _jieba_seg(self, text_treated):
        """
        传入预处理完成的文本
        jieba分词
        返回分词完成的list
    ...
    def _get_stopWords(self):
        ...
        获取停用词
        """
        if os.path.exists(self._file_path_of_stopWords) == False:
            print('停用词文件不存在!!!')
            exit(1)
