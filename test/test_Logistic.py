...
import sys
sys.path.append("../")
from SynXNLP.classify.Logistic import Logistic
...
def load_data_set():
    ...
    train_data = [[1.0, x1, x2] for x1, x2, label in data]
    labels = [label for x1, x2, label in data]
...
def main():
    ...
    for i in range(len(labels)):
        result = C.predict(train_data[i])
        if int(labels[i]) == 1:
            if result == 1:
                #print('NO.%2d被正确识别 label:%d Position:(%s,%s)' % (i+1, result, train_data[i][1], train_data[i][2]))
                TP += 1
            else:
                print('NO.%2d被错误识别 label:%d Position:(%s,%s)' % (i+1, result, train_data[i][1], train_data[i][2]))
                FN += 1
        else:
            if result == 0:
                #print('NO.%2d被正确识别 label:%d Position:(%s,%s)' % (i+1, result, train_data[i][1], train_data[i][2]))
    ...
    print('='*120)
    print('weight:%s' % C.train_result())
    print("TP: %d\tFN: %d\tTN: %d\tFP: %d" % (TP, FN, TN, FP))
    print('Precision:  %f' % (TP/(TP+FP) * 100))
    print('Recall:  %f' % (TP/(TP+FN) * 100))
    print('F-Measure:  %f' % ((2 * TP/(TP+FP) * TP/(TP+FN) / (TP/(TP+FP) + TP/(TP+FN))) * 100))
    print('='*120)

    plot_graph(train_data, labels, C.train_result())

def plot_graph(data, labels, weights):
